...
*/
   ...
   public String decodeString(StringPart[] parts)
   {
      ...
      for(int k=0; k<parts.length; k++){
         decoded = decoded + masterString.substring(parts[k].getStart(), 
            parts[k].getStart()+parts[k].getLength());
      }
      return decoded;
   }
...
// Part B
   public StringPart[] encodeString(String word)
   {
     StringPart[] temp = new StringPart[100];
         for(int index = 0; word.length() > 0 ;index++)
         {
         temp[index] = findPart(word);
         word = word.substring(temp[index].getLength());
         }
